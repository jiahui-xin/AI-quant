(function () {
  const dict = {
    ...
    "TASK5": "TASK5",
    "进入 TASK5": "Open TASK5",
    "AI 交易引擎：分类型机器学习算法与场景应用": "AI Trading Engine: Classification ML Algorithms and Applications",
    "使用 A 股财务指标收益数据集，构建决策树与随机森林分类模型预测股票涨跌方向，并通过混淆矩阵、AUC 与 ROC 曲线评估模型表现。": "Build Decision Tree and Random Forest classifiers on A-share financial-indicator data to predict stock return direction, and evaluate with confusion matrix, AUC, and ROC curves.",
    "查看 TASK5 分类机器学习": "View TASK5 Classification ML",
    "进入模型实验室": "Open Model Lab",
    "分类型机器学习算法与评价指标": "Classification ML Algorithms and Evaluation Metrics",
    "模型实验室": "Model Lab",
    "测试集混淆矩阵": "Test-set Confusion Matrix",
    "TASK6": "TASK6",
    "进入 TASK6": "Open TASK6",
    "智能决策者：用机器学习定制专属策略": "Smart Decision Maker: Custom Strategies with Machine Learning",
    "用 A 股截面财务因子训练四种机器学习模型，预测股票下一季度收益，每季度按预测得分挑选 Top 30 股票构建等权与预测加权组合，对比四模型与市场基准。": "Train four ML models on A-share cross-sectional financial factors to predict next-quarter returns, build equal-weight and predicted-weight Top 30 portfolios each quarter, and compare all four models against the market benchmark.",
    "查看 TASK6 ML 选股": "View TASK6 ML Stock Selection",
    "进入选股实验室": "Open Stock Selection Lab",
    "分类型回归模型与选股逻辑": "Regression Models and Stock Selection Logic",
    "选股实验室": "Stock Selection Lab",
    "特征重要性与回归系数": "Feature Importance and Regression Coefficients",
